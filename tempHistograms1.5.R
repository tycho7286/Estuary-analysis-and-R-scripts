...
### Group selectors:
### - c("all") keeps every available value as a separate group.
### - c("combined") pools every available value into one group.
### - c("value1", "value2") includes only those values as separate groups.
...
### Stack temperature selector:
### Valid values are "mean", "max", "median", and "min".
### This changes the temperature time series and temperature distribution
### in the three-panel stacked plot. The PCA CSV always contains all five
### statistics regardless of this selection.
strSelectedTemperatureStatistic <- "mean"

### Stack distribution selector:
### Use "hist" to draw histograms in both distribution panels.
### Use "fp" to draw frequency polygons in both distribution panels.
strStackDistributionPlotType <- "fp"
...
### Make shared absolute breaks aligned to zero.
makeHistogramBreaks <- function(valueLimits, binWidth) {
  minimumBreak <- floor(valueLimits[1] / binWidth) * binWidth
  maximumBreak <- ceiling(valueLimits[2] / binWidth) * binWidth

  if (maximumBreak <= valueLimits[2]) {
    maximumBreak <- maximumBreak + binWidth
  }

  seq(minimumBreak, maximumBreak, by = binWidth)
}

### Return counts and proportions for one group's shared histogram bins.
makeHistogramCSVRows <- function(values, histogramBreaks) {
  ...
  data.frame(
    binLower = head(histogramBreaks, -1),
    binUpper = tail(histogramBreaks, -1),
    binCenter = (
      head(histogramBreaks, -1) + tail(histogramBreaks, -1)
    ) / 2,
    count = histogramCounts,
    proportion = histogramCounts / sum(histogramCounts)
  )
}
...
strSelectedTemperatureStatistic <- trimws(
  tolower(strSelectedTemperatureStatistic)
)
vectorValidTemperatureStatistics <- c("mean", "max", "median", "min")

if (!strSelectedTemperatureStatistic %in% vectorValidTemperatureStatistics) {
  stop(
    "strSelectedTemperatureStatistic must be one of: ",
    paste(vectorValidTemperatureStatistics, collapse = ", ")
  )
}

strSelectedTemperatureColumn <- paste0(
  strSelectedTemperatureStatistic,
  "Temp"
)
strSelectedTemperatureTitle <- tools::toTitleCase(
  strSelectedTemperatureStatistic
)

strStackDistributionPlotType <- trimws(
  tolower(strStackDistributionPlotType)
)
if (!strStackDistributionPlotType %in% c("hist", "fp")) {
  stop("strStackDistributionPlotType must be either 'hist' or 'fp'.")
}
...
cat(
  ...
  "  Selected temperature statistic: ",
  strSelectedTemperatureStatistic,
  "\n",
  "  Stack distribution plot type: ",
  strStackDistributionPlotType,
  "\n",
  sep = ""
)
...
strHistogramPCACSVName <- file.path(
  strImagePath,
  "histogramPCAData.csv"
)

if (file.exists(strHistogramPCACSVName)) {
  file.remove(strHistogramPCACSVName)
}

bolHistogramPCAHeaderWritten <- FALSE
...
vectorTemperatureHistogramBreaks <- makeHistogramBreaks(
  temperatureLimits,
  dblTemperatureBinWidth
)
vectorRangeHistogramBreaks <- makeHistogramBreaks(
  temperatureRangeLimits,
  dblTemperatureRangeBinWidth
)
...
cat(
  ...
  "Mean, maximum, median, minimum, and range histogram bins will be\n",
  "appended to histogramPCAData.csv.\n",
  "Output groups:\n",
  sep = ""
)
...
for (i in seq_len(nrow(dfPlotGroups))) {
  ...
  listHistogramPCARows <- lapply(
    c("max", "min", "median", "mean"),
    function(strHistogramStatistic) {
      makeHistogramCSVRows(
        dfWindowSummary[[paste0(strHistogramStatistic, "Temp")]],
        vectorTemperatureHistogramBreaks
      ) %>%
        mutate(statistic = strHistogramStatistic, .before = 1)
    }
  )

  listHistogramPCARows[[5]] <- makeHistogramCSVRows(
    dfWindowSummary$tempRange,
    vectorRangeHistogramBreaks
  ) %>%
    mutate(statistic = "range", .before = 1)

  dfHistogramPCACSV <- bind_rows(listHistogramPCARows) %>%
    mutate(
      region = strGroupRegion,
      estuary = strGroupEstuary,
      station = strGroupStation,
      profile = strGroupProfile,
      season = strGroupSeason,
      windowCount = nrow(dfWindowSummary),
      .before = 1
    )

  write.table(
    dfHistogramPCACSV,
    file = strHistogramPCACSVName,
    sep = ",",
    row.names = FALSE,
    col.names = !bolHistogramPCAHeaderWritten,
    append = bolHistogramPCAHeaderWritten,
    quote = TRUE,
    na = ""
  )

  bolHistogramPCAHeaderWritten <- TRUE
  ...
  ############################################################
  ### Selected Temperature Statistic and Range Plot Stack
  ############################################################

  plotSelectedTemperatureTimeSeries <- ggplot(
    dfWindowSummary,
    aes(
      x = windowCenterDateTime,
      y = .data[[strSelectedTemperatureColumn]]
    )
  ) +
    geom_line(linewidth = 0.3, color = "steelblue") +
    labs(
  ...
  if (strStackDistributionPlotType == "hist") {
    strStackDistributionTitle <- "Histogram"
    strStackDistributionFilename <- "Hist"

    plotRangeDistribution <- ggplot(
      dfWindowSummary,
      aes(x = tempRange)
    ) +
      geom_histogram(
    ...
    plotSelectedTemperatureDistribution <- ggplot(
      dfWindowSummary,
      aes(x = .data[[strSelectedTemperatureColumn]])
    ) +
      geom_histogram(
        binwidth = dblTemperatureBinWidth,
        boundary = 0,
    ...
    strStackDistributionTitle <- "Frequency Polygon"
    strStackDistributionFilename <- "FP"
  ...
  plotRangeDistribution <- plotRangeDistribution +
    coord_cartesian(xlim = temperatureRangeLimits) +
    labs(
      title = paste0(
        strGroupEstuary,
        ": Temperature Range ",
        strStackDistributionTitle
      ),
      subtitle = paste0(
        strGroupSubtitle,
        " | ",
  ...
  plotSelectedTemperatureDistribution <-
    plotSelectedTemperatureDistribution +
    coord_cartesian(xlim = temperatureLimits) +
    labs(
      title = paste0(
  ...
  plotTimeSeriesRangeTemperatureStack <- plotSelectedTemperatureTimeSeries /
  ...
  ggsave(
    ...
    plot = plotTimeSeriesRangeTemperatureStack,
    width = intImageWidth,
  ...
  rm(
    ...
    dfWindowSummary,
    dfWindowOutput,
    listHistogramPCARows,
    dfHistogramPCACSV,
    plotSelectedTemperatureTimeSeries,
    plotRangeDistribution,
    plotSelectedTemperatureDistribution,
    plotTimeSeriesRangeTemperatureStack
  )
  gc()
}
...
if (bolHistogramPCAHeaderWritten) {
  cat(
    "Wrote max, min, median, mean, and range histogram bins to: ",
    strHistogramPCACSVName,
    "\n",
    sep = ""
  )
} else {
  cat("No groups passed the cutoff, so no histogram PCA CSV was written.\n")
}
